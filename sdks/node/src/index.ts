...
class StatusKeet {
  ...
  private manualStatuses = new Map<string, ServiceStatus>();

  constructor(options: StatusKeetOptions) {
    if (!options.apiKey) throw new Error("StatusKeet: apiKey is required");
  ...
  private async register(): Promise<void> {
    ...
    if (!res.ok) {
      const err = await res.text();
      throw new Error(
        `StatusKeet service registration failed (${res.status}): ${err}`
      );
    }
  }

  private async collectStatuses(): Promise<ServiceStatus[]> {
